...
[cmdletbinding()]
param($InitialCommand = $null, [switch] $NoNewShell, [switch] $Wait, [switch] $ReuseConsole, [switch] $FromSource, [switch] $ReturnExitCode, [switch] $NoImport, $Path, [switch] $AllowProfile, [switch] $PSProfile)
...
if (! $NoNewShell.ispresent ) {
    ...
    $processStatus = if ( ! $NoNewWindow ) {
        start-process $PowerShellExecutable $shellArguments -Wait:$shouldWait -passthru
    } else {
        start-process $PowerShellExecutable $shellArguments -Wait:$shouldWait -nonewwindow -passthru
    }
