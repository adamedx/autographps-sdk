...
<#
.SYNOPSIS
Finds certificates in the local operating system certificate store (Windows only) for use as Azure Active Directory application credentials to access the Graph API.

.DESCRIPTION
Find-GraphLocalCertificate searches the operating system's certificate store for certificates that can be used as credentials for Azure Active Directory (AAD) application identities. When such a certificate is configured as an allowed credential for an AAD application that is enabled for Graph API access, they can be specified to commands like Connect-GraphApi, New-GraphConnection, or as part of profile settings configuration to allow the module's commands to issue requests to the Graph API. Currently the only operating system with a certificate store supported by Find-GraphLocalCertificate is Windows.

The command supports searching for the certificate by AAD application identifier, AAD object identifier, and as a partial match of the application's display name. The latter capability is not considered authoritative since application display names are user-defined and are not unique, but the ability to search by display name is offered as a user experience optimization for human interaction scenarios since operators often recognize applications and their purposes or importance by the application display name.

The command may also be executed without parameters, in which case it returns all certificates in the certificate store which it identifies as credentials used to access AAD applications targeting the Graph API.

Find-GraphLocalCertificate returns only those certificates in the certificate store that contain a given AAD application's application identifier or object identifer as part of the certificate's subject name or friendly name. Commands like New-GraphApplicationCertificate and New-GraphLocalCertificate create application credential certificates in the certificate store using just such a convention. Thus, Find-GraphLocalCertificate is optimized for management of certificates created by those commands. See the notes section for more details on how Find-GraphLocalCertificate associates an application with the certificate it requires to authenticate its identity.

The Find-GraphLocalCertificate command is useful in several situations:
   * The module's implicit and explicit sign-in functionality utilizes the naming convention described above for certificates created by New-GraphLocalCertificate and New-Graphcertificate to find the correct credential certificate for a given application identifier (or its AAD object identifier). You can use Find-GraphLocalCertificate to exercise the same search capability. If you need to troubleshoot why a sign-in appears not to use the correct certificate, Find-GraphLocalCertificate shows the certificates that the sign-in functionality is discovering so you can narrow down the the problem.
   ...
   * When you remove an application from AAD using a command like Remove-GraphApplication, if there are local certificates for that application on the current system they are not removed by Remove-GraphApplication. To remove them, you need to find the certificate and pass its path to Remove-Item. You can find the application for a given certificate using Find-GraphLocalCertificate.
...
.PARAMETER AppId
The AAD application identifier of the application for which to search for certificates. Specify this parameter to find certificates associated with the specified application identifier.

.PARAMETER Name
Specify the Name parameter to search for certificates by the application's display name (in AAD this corresponds to the application's displayName property, though the certificate will only be stamped with a snapshot of that property as it existed at the time the certificate was created). Any certificates with a FriendlyName field that contains a partial match of the value specified for Name will be returned by Find-GraphLocalCertificate.

.PARAMETER ObjectId
The AAD object identifier of the application, distinct from its application identifier, for which to search for certificates. Specify this parameter to find certificates associated with the application with the specified object identifier.
...
The convention that is to identify a certificate as being bound to an AAD application identity is as follows:
   ...
   * The certificate's Subject property must have the format 'CN=<appid>, CN=AutoGraphPS, CN=MicrosoftGraph` where <appid> is the AAD application identifier guid for the application
   ...
   * To find the application based on its AAD object id identifier, the above requirement on the format of the Subject property still holds, but the application identifier constraint is not evaluated. Instead, an additional requirement is that the FriendlyName property must contain the application's AAD object identifier as a substring.
...
An example of a certificate generated by New-GraphApplicationCertificate is given below:
    ...
    FriendlyName : Credential for Microsoft Graph Azure Active Directory application name='ImageBackup',
...
In this example, an AAD application identifier is specified using the AppId parameter, which is specified here without the parameter name since it is also the first positional parameter. The single match is output to the console.
...
Here Find-GraphLocalCertificate is specified without any parameters, so it returns all certificates that appear to be associated withy any AAD application that are found under the default search path cert:/currentuser/my. The results are piped to Measure-Object and Select-Object parameters to emit the number of such certificates to the console.

.EXAMPLE
Find-GraphLocalCertificate 4fe0f58d-457a-4f84-8658-3b29f14d4723 | Remove-Item

When a certificate for an application is no longer needed, such as in the situation where the AAD application is deleted from AAD entirely, Find-GraphLocalCertificate can be used to find its certificate in the certificate store. In this example, the result of such an invocation of Find-GraphLocation is piped to the standard PowerShell command Remove-Item which deletes it from the certificate store.
...
This example shows how the command may be used to find an application's certificate based on the application's AAD object identifier rather than the application identifier.
