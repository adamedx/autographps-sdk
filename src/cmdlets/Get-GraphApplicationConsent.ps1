...
.PARAMETER ObjectId
Specify the ObjectId parameter to return consent objects for permissions granted in the organization to the application with the specified object identifier.
...
Currently the output of Get-GraphApplicationConsent is a custom object rather than one of the types of consent object returned by the Graph API. The native object is accessible via th GraphResource property of the custom object, and is either an oauth2PermissionGrant or appRoleAssignment resource documented in the Graph API reference. This may change in future iterations of the command, in which case the returned object will itself be the native consent object (i.e. oauth2PermissionGrant or appRoleAssignment) and there will be no GraphResource property, introducing a breaking change.
